...
function charSpellout (_names){

    //creating an outer loop
    for (let k=0; k<_names.length; k++)
    {
        console.log(`My name is ${_names[k]}`);
        //creating an inner loop
        for (let l=0; l<_names[k].length; l++)
        {
            //create a statement for the inner loop
            console.log(_names[k][l]);
        }
    }
...
let arrNames = ["Jasper", "Gian", "Vincent"];

function spellOut (arrNames) {
    for (let i = 0; i<arrNames.length; i++) {
        console.log(`My name is ${arrNames[i]}`);

        for (let j=0; j<arrNames[i].length; j++){
            console.log(`Spell is ${arrNames[i][j]}`);
        }
    }
}
